...
public class Testing {
    ...
    public void testSynchronizeFrontCase() throws InterruptedException {
        commitAll(repo1, new String[]{"Old1", "Old2"});
        commitAll(repo2, new String[]{"New1", "New2"});

        repo1.synchronize(repo2);

        assertEquals(4, repo1.getRepoSize());
        assertEquals(0, repo2.getRepoSize());
        testHistory(repo1, 4, new String[]{"Old1", "Old2", "New1", "New2"});
    } 
    ...
    public void testSynchronizeMiddleCase() throws InterruptedException {
        commitAll(repo1, new String[]{"Old"});
        commitAll(repo2, new String[]{"Middle"});
        commitAll(repo1, new String[]{"New"});

        repo1.synchronize(repo2);

        assertEquals(3, repo1.getRepoSize());
        assertEquals(0, repo2.getRepoSize());
        testHistory(repo1, 3, new String[]{"Old", "Middle", "New"});
    } 
    ...
    @Test
    @Timeout(1)
    @DisplayName("Test - Synchronize Empty Case")
    public void testSynchronizeEmptyCase() throws InterruptedException {
        ...
        assertEquals(0, repo2.getRepoSize());
        testHistory(repo1, 2, new String[]{"First", "Second"});
    } 
